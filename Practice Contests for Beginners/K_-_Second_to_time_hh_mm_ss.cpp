...
int main(){
    ...
    int n;
    cin>>n;
    if(n<0) cout<<"total second must be a positive integer"<<endl;
    else{
        if(n/3600>1) cout<<n/3600<<":"<<(n%3600)/60<<":"<<((n%3600)%60)%60<<1;
        else if(n/3600<1 && n/60>1) cout<<00<<":"<<n/60<<":"<<(n/60)%60;
        else cout<<00<<":"<<00<<":"<<n%60<<3;
    }
    return 0;
}